...
apt-get install -y -qq build-essential cmake ninja-build git pkg-config \
  p7zip-full curl ca-certificates file wget squashfs-tools patchelf zsync \
...
echo "######## 2. modern SDL2 (Debian 12 ships 2.26 with broken Wayland fullscreen) ########"
git config --global --add safe.directory '*' || true
git clone -q --depth 1 -b SDL2 https://github.com/libsdl-org/SDL.git /tmp/sdl2-src
...
echo "######## 5. free FX data (official release; already free of the 16 copyright files) ########"
DATA_7Z="keeperfx_${KFX_DATA_VERSION//./_}_complete.7z"
curl -Lso "/tmp/$DATA_7Z" \
  "https://github.com/dkfans/keeperfx/releases/download/v${KFX_DATA_VERSION}/${DATA_7Z}"
...
echo "######## 9. appimagetool ########"
VER="$(grep -E '^VER_MAJOR=' version.mk | cut -d= -f2).$(grep -E '^VER_MINOR=' version.mk | cut -d= -f2).$(grep -E '^VER_RELEASE=' version.mk | cut -d= -f2)"
cd /tmp
